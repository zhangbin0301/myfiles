...
if source /root/env.yml; then
  ...
  splithttp_url="vless://${UUID}@${CF_IP}:${splitport}?path=%2Fsplit&security=tls&encryption=none&host=${MY_DOMAIN}&type=splithttp&sni=${MY_DOMAIN}#${country_abbreviation}-${SUB_NAME}-splithttp"
